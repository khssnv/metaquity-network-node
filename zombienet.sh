...
zombienet_spawn() {
  zombienet_init
  if [ ! -f target/release/metaquity-network-node ]; then
    echo "building metaquity-network-node..."
    cargo build --release
  fi
  echo "spawning polkadot-local relay chain plus metaquity-network-node..."
...
print_help() {
  ...
  echo "$ ./zombienet.sh spawn        # spawns a rococo-local relay chain plus metaquity-network-node"
}
