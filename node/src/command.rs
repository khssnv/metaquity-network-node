...
use log::{info, warn};
use metaquity_network_runtime_common::Block;
use sc_cli::{
		...
		match runner.config().chain_spec.runtime() {
			Runtime::Devnet | Runtime::Default => {
				runner.async_run(|$config| {
					let $components = new_partial::<metaquity_network_devnet_runtime::RuntimeApi, DevnetRuntimeExecutor, _>(
						&$config,
						crate::service::build_import_queue::<metaquity_network_devnet_runtime::RuntimeApi, DevnetRuntimeExecutor>,
					)?;
			...
			Runtime::Mainnet => {
				runner.async_run(|$config| {
					let $components = new_partial::<metaquity_network_mainnet_runtime::RuntimeApi, MainnetRuntimeExecutor, _>(
						&$config,
						crate::service::build_import_queue::<metaquity_network_mainnet_runtime::RuntimeApi, MainnetRuntimeExecutor>,
					)?;
					let task_manager = $components.task_manager;
					{ $( $code )* }.map(|v| (v, task_manager))
				})
...
macro_rules! construct_benchmark_partials {
	($config:expr, |$partials:ident| $code:expr) => {
		match $config.chain_spec.runtime() {
			Runtime::Devnet | Runtime::Default => {
				let $partials = new_partial::<metaquity_network_devnet_runtime::RuntimeApi, DevnetRuntimeExecutor, _>(
					&$config,
			...
			Runtime::Mainnet => {
				let $partials =
					new_partial::<metaquity_network_mainnet_runtime::RuntimeApi, MainnetRuntimeExecutor, _>(
						&$config,
			...
			runner.run_node_until_exit(|config| async move {
				...
				match config.chain_spec.runtime() {
					Runtime::Default | Runtime::Devnet => {
						...
						sp_core::crypto::set_default_ss58_version(metaquity_network_devnet_runtime::SS58Prefix::get().into());
						crate::service::start_parachain_node::<metaquity_network_devnet_runtime::RuntimeApi, DevnetRuntimeExecutor>
					...
					Runtime::Mainnet => {
						sp_core::crypto::set_default_ss58_version(metaquity_network_mainnet_runtime::SS58Prefix::get().into());
						crate::service::start_parachain_node::<metaquity_network_mainnet_runtime::RuntimeApi, MainnetRuntimeExecutor>
							(config, polkadot_config, collator_options, id, hwbench)
							.await
							.map(|r| r.0)
							.map_err(Into::into)
